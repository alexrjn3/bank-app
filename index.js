...
const login = document.querySelector('.login-form').addEventListener('submit', function (e) {
    ...
    for (let i = 0; i < persons.length; i++) {
        if (login_user === persons[i].user && login_pin === persons[i].pin) {
            ...
            if (login_user === "" || login_pin === "") {
                //errorMessage.innerHTML = "<strong>USER</strong> or <strong>PIN</strong> not completed";
            }
...
const logoutf = function () {
    ...
    const scrollable_content = document.querySelector('.scrollable-content');
    while (scrollable_content.firstChild) {
        scrollable_content.removeChild(scrollable_content.firstChild);
    }
    ...
    void scrollable_content.offsetWidth;  // Trigger reflow
...
const updateHistory = function (transferDa, amount) {
    ...
    let transaction = {
        ...
        date: get_date()
        //transactionNumber: currentPerson.counterTranzactie //nu cred ca ne mai trb, deja avem coutner global in obiect
    };
...
const transferReusit = function (transferToPerson, amount_transfer) {
    ...
    let transaction = {
        ...
        date: get_date()
        //transactionNumber: transferToPerson.counterTranzactie
    };
...
const transfer_money = document.querySelector('.transfer-form').addEventListener('submit', function (e) {
    ...
    for (let i = 0; i < persons.length; i++) {
        if (transfer_person === persons[i].user) {
            if (amount_transfer < 3000) {
                ...
                if (currentPerson.transfer_premium === false) {
...
const loan_money = document.querySelector('.loan-form').addEventListener('submit', function (e) {
    ...
    let loan_amount;
    if (butonLoan_setat_payLoan == false) {
        loan_amount = parseFloat(document.querySelector('.amount-input-loan').value);
    }
    else {
        loan_amount = parseFloat(document.querySelector('.options-loans').value);
    }
    if (!isNaN(loan_amount)) {
        ...
        if (amountInputLoan) { // Check if the element exists
            amountInputLoan.value = ""; // Clear the value
        }
    }
    else {
        document.querySelector('.wrong-cred-loan').classList.remove("hidden-class");
...
const close_acc = document.querySelector('.close-form').addEventListener('submit', function (e) {
    ...
    //delete
    for (let i = 0; i < persons.length; i++) {
        if (close_user === persons[i].user && close_pin === persons[i].pin) {
            wrongPerson = 0;
...
const sortbtn = document.querySelector('.sort').addEventListener('click', function () {
    ...
    const sortbuton = document.querySelector('.sort');
    let aux = [];
    for (let i = scrollable_content.children.length - 1; i >= 0; i--) {
        aux.push(scrollable_content.children[i]);
    }
    for (let i = 0; i < aux.length; i++) {
        scrollable_content.appendChild(aux[i]);
    }
